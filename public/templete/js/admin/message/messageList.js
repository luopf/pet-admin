$(function(){
    ...
    function getSelectInfo(){
        ...
        var selectInfo = {
            mess_num : $.trim($("#mess_num").val()),//消息编号
            nick_name : $.trim($("#nick_name").val()),//用户名
            phone : $.trim($("#phone").val()),// 联系电话
            label_name : $.trim($("#label_name").val()),//标签名
            state : $('#state').val(),//订单状态
            address_text : $.trim($("#address_text").val()),
            from : $('#startTime').val(),
            to : $('#endTime').val(),
            isdelete : 0
        };
    ...
    function render(async,pageIndex,pageSize){
        ...
        $.ajax({
            async:async,
            type:'post',
            url:host+'/index.php/admin/message/pagingMessage',
            ...
            success:function(result){
                ...
                if(result.errorCode == 0){
                    ...
                    html+='<tr><th class="th1"><input type="checkbox" class="select-all my-icheckbox"></th><th class="th1">序号</th><th class="th2">用户名</th><th class="th3">消息编号</th><th class="th4">标签</th><th class="th8">发布需求内容</th><th class="th5">联系方式</th><th class="th6">地址</th><th class="th12">发布状态</th><th class="th9">发布时间</th><th class="th10">操作</th></tr>';
                    var colspan = $(html).find("th").length;
                    for(var i = 0; i < myList.length;i++){
                        var obj = myList[i];
                        var num = (pageIndex-1)*pageSize + i+1;// 序号
                        var mess_num = obj.mess_num || "--";// 消息编号
                        var nick_name = obj.nick_name || "--";// 用户名
                        var label_name = obj.label_name || "--";// 标签名
                        var text_content = obj.text_content || "--";// 用户发布的内容
                        var phone = obj.phone || "__";// 用户的联系方式
                        var address_text = obj.address_text || "--"; //用户的联系地址
                        var state = obj.state;// 发布消息的状态
                        var state_text= '';
                        if(state == 0){
                            state_text = "未审核";
                        } else if(state == 1){
                        ...
                        state = setState(obj.state,express_code,express_number);
                        ...
                        html+='<tr>'
                            ...
                            +'<td class="th3"><a href="javascript:;" title="查看订单详情" class="order-detail" oid="'+oid+'">'+mess_num+'</a></td>'
                            +'<td class="th4">'+label_name+'</td>'
                            +'<td class="th4">'+text_content+'</td>'
                            +'<td class="th5">'+phone+'</td>'
                            +'<td class="th6"><span data-toggle="popover" data-trigger="click" data-placement="top" title="' + addressInfo +'">' + address_text + '</span></td>'
                            +'<td class="th11"><span data-toggle="popover" data-trigger="click" data-placement="top" title="' + contact_time +'">'+state_text+'</span></td>'
                            +'<td class="th12">'+add_time+'</td>'
                            +'<td>'+operation+'</td>'
                            +'</tr>';
                    }
                    if(myList.length == 0){
                        ...
                        $(".modify-message").click(function(){
                        	var oid = $(this).attr("oid");
                        	window.location.href = getJSurl()+"/index.php/admin/message/messageDetail/id/"+oid;
                        });
                        //删除订单
    ...
    function deleteMessage(){
        ...
        myConfirmModal("确定删除当前订单吗？",function(){
            $.ajax(
                {
                    type:"post",
                    url:host+'/index.php/admin/message/deleteMessage',
                    data:{"id":oid},
                    dataType:"json",
                    beforeSend:function(xhr){
                        //显示“加载中。。。”
