$(function(){
    ...
    function bindEvent(){
        ...
        //添加文章
        $("#add_aticle").click(function () {
            window.location.href = host+"/index.php/admin/Petaticle/addAticle";
        })
    ...
    function render(async,pageIndex,pageSize){
        ...
        $.ajax({
            async:async,
            type:'post',
            url:host+'/index.php/admin/Petaticle/paggingAticle',
            ...
            success:function(result){
                ...
                if(result.errorCode == 0){
                    ...
                    html+='<tr><th class="th1"><input type="checkbox" class="select-all my-icheckbox"></th><th class="th1">序号</th><th class="th2">作者</th><th class="th3">标签名</th><th class="th4">图片</th><th class="th8">标题</th><th class="th9">发布时间</th><th class="th10">操作</th></tr>';
                    ...
                    for(var i = 0; i < myList.length;i++){
                        ...
                        var img_url = obj.img_url || '--';// 图片
                        var thumb = obj.thumb || '--';//缩略图
                        var title = obj.title || '--';// 标题
                        var aticle_desc = obj.aticle_desc || '--';// 文章简介
                        var add_time = obj.add_time || '--';// 添加时间
                        ...
                        html+='<tr>'
                            ...
                            +'<td class="th1">'+num+'</td>'
                            +'<td class="th2"><a href="javascript:;" title="查看用户详情" class="user-detail" >'+author+'</a></td>'
                            +'<td class="th3"><a href="javascript:;" title="查看订单详情" class="order-detail" oid="'+oid+'">'+label_name+'</a></td>'
                            +'<td class="th4"><img src="'+upload+thumb+'"/></td>'
                            +'<td class="th4">'+title+'</td>'
                    ...
                    if(myList.length == 0){
                        ...
                        //查看文章详情
                        $(".aticleDetail").click(function(){
                            var oid = $(this).attr('oid');
                            window.location.href = host+"/index.php/admin/petaticle/aticleDetail/id/"+oid;
                        });
                        ...
                        //删除订单
                        $(".deleteAticle").click(deleteAticle);
...
//                        	var callBackUrl = "http://192.168.1.124/cws/admin.php?c=store_order_a_orderList_mid_29";
    ...
    function deleteAticle(){
        var oid = $(this).attr('oid');
        myConfirmModal("确定删除当前文章吗？",function(){
            $.ajax(
                {
                    type:"post",
                    url:host+'/index.php/admin/petaticle/deleteAticle',
                    data:{"id":oid},
                    dataType:"json",
                    beforeSend:function(xhr){
                        //显示“加载中。。。”
    ...
    function orderOperatoin(state,oid){
        ...
        switch (state)
        {
            case '0'://未审核
                ...
                break;
            case '1'://已审核
            	html +="<a  href='javascript:;' oid ='"+ oid+"' class='btn btn-primary btn-xs aticleDetail'title='设置'>查看</a>";
                html+="<a  href='javascript:;'  oid ='"+oid+"' class='btn btn-danger btn-xs deleteAticle' title='删除文章'>删除</a>";;
                break;
            case '2'://审核失败
            	html +="<a  href='javascript:;' oid ='"+ oid+"' class='btn btn-primary btn-xs modify-message'title='设置'>查看</a>";
